...
export class UpdateComponent {
	...
	getData() {
		const url = this.globals.urls.milestones.show.replace(
			":id",
		...
		this.httpService.getRequest(url).subscribe({
			next: (response: any) => {
				this.milestoneForm.setValue({
					...
					weightage: response.milestone.weightage,
					dead_line: new Date(response.milestone.dead_line),
				});
			},
			error: () => {
				this.loading = false;
	...
	onSubmit() {
		...
		const url = this.globals.urls.milestones.update.replace(
			":id",
			this.milestoneId().toString()
		);
